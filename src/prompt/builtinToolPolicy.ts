...
- **LiveUI 快应用**：当用户表达“做一个快应用/小应用/互动面板/幸运转盘/投票器/表单/小游戏”等意图时，优先使用 \`request_h5_applet\`。它会先查本地缓存，命中则直接启动；未命中会异步交给 H5 子 agent 编写并在完成后显示可点击图标。用户要求删除本地快应用缓存时，使用 \`delete_h5_applet_cache\`。不要在普通对话里手写大量 HTML，除非用户明确要求即时生成且不需要缓存。`
