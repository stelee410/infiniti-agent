...
async function appendJobLog(cwd: string, line: string): Promise<void> {
  try {
    ...
    await appendFile(join(dir, 'video-async.log'), `${new Date().toISOString()} ${line}\n`, 'utf8')
  } catch {
...
function currentCliInvocation(cwd: string, jobPath: string): { command: string; args: string[] } {
  ...
  if (!entry) {
    return { command: 'infiniti-agent', args: ['video-worker', jobPath] }
  }
  if (/\.(tsx?|mts|cts)$/i.test(entry)) {
    ...
    return {
      ...
      args: existsSync(localTsx)
        ? [entry, 'video-worker', jobPath]
        : ['tsx', entry, 'video-worker', jobPath],
    }
  }
  return {
    command: process.execPath,
    args: [...process.execArgv, entry, 'video-worker', jobPath],
  }
}
