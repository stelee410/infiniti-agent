...
### 0.1 窗口形态：默认普通应用窗口（去精灵化）

"桌面精灵"范式（透明悬浮、置顶、鼠标穿透、可缩成极简）对普通用户是负担——复杂任务不够用、简单任务又冗余。因此 **Live 默认改为普通应用窗口**：有边框、不透明、不置顶、不穿透、可缩放，形象与对话/活动都装在常规窗口内。

桌宠/精灵模式**未删除，保留在开关后**，默认关：

```
INFINITI_LIVEUI_SPRITE_WINDOW=1   # 启用桌宠/精灵叠层
```
