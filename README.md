...
**Electron 调试窗口（默认）**：带系统标题栏与 **视图** 菜单（重新加载、开发者工具、缩放等）。恢复早期无边框置顶叠层：`INFINITI_LIVEUI_FRAMELESS=1`。启动时自动打开 DevTools（独立窗口）：`INFINITI_LIVEUI_DEVTOOLS=1`（需在启动 Electron 前设置环境变量，可在后续 CLI 中接入）。
