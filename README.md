...
- `subconsciousHeartbeatMs`：subconscious-agent heartbeat 间隔，单位毫秒；默认 `60000`，范围 `5000`～`3600000`。
- `figureZoom`：LiveUI 虚拟人缩放比例，范围 `0.4`～`1.5`；`infiniti-agent live --zoom` 可临时覆盖。
- `renderer`：角色渲染方式，可选 `live2d`、`sprite`、`real2d`。未设置时保持旧逻辑：有 `spriteExpressions.dir` 则用 `sprite`，否则用 `live2d`。
- `spriteExpressions.dir`：`sprite` 模式使用含 `exp_01.png`…`exp_08.png` 的目录（如 `./live2d-models/luna/expression`）；`real2d` 模式按 `/real2d` 参考代码使用含 `exp01.png`…`exp06.png`、可选 `exp_open.png` 的目录（如 `./live2d-models/jess/expression2d`），并通过 MediaPipe landmarks 做实时 2D 变形与口型驱动。
